...
// 内嵌浏览器所带的方法
let external: any = win.external

let userAgent = win.navigator.userAgent
let appVersion = win.navigator.appVersion
let vendor = win.navigator.vendor

let detector: any = {}

let re_msie = /\b(?:msie |ie |trident\/[0-9].*rv[ :])([0-9.]+)/
...
const BROWSER = [
  ...
  [
    'theworld',
    function () {
      let x = checkTW360External('theworld')
  ...
  [
    '360',
    function (ua: any) {
      let x = checkTW360External('360se')
      if (typeof x !== 'undefined') {
        return x
      }
  ...
  [
    'opera',
    function (ua: any) {
      let re_opera_old = /\bopera.+version\/([0-9.ab]+)/
      let re_opera_new = /\bopr\/([0-9.]+)/
...
function checkTW360External (key: string) {
  ...
  try {
    ...
    let runpath = external.twGetRunPath.toLowerCase()
    // 360SE 3.x ~ 5.x support.
    // 暴露的 external.twGetVersion 和 external.twGetSecurityID 均为 undefined。
    // 因此只能用 try/catch 而无法使用特性判断。
    let security = external.twGetSecurityID(win)
    let version = external.twGetVersion(security)
